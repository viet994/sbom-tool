...
namespace Microsoft.Sbom.Api.Config
{
    ...
    /// </summary>
    public class ContextPostProcessor : IMappingAction<ContextAdapter, ContextAdapter>
    {
        private readonly IEnumerable<ConfigValidator> configValidators;
        private readonly ContextSanitizer contextSanitizer;

        public ContextPostProcessor(IEnumerable<ConfigValidator> configValidators, ContextSanitizer contextSanitizer)
        {
            this.configValidators = configValidators ?? throw new ArgumentNullException(nameof(configValidators));
            this.contextSanitizer = contextSanitizer ?? throw new ArgumentNullException(nameof(contextSanitizer));
        }

        public void Process(ContextAdapter source, ContextAdapter destination, ResolutionContext context)
        {
            ...
            destination = contextSanitizer.SanitizeContext(destination);
        }

        private void SetDefautValue(IContext destination, object value, PropertyDescriptor property)
        {
