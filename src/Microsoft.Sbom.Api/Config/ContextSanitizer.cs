...
{
    /// <summary>
    /// Sanitizes a validated context by setting additional parameters or fixing default parameters if needed.
    /// </summary>
    public class ContextSanitizer
    {
        ...
        public ContextSanitizer(IHashAlgorithmProvider hashAlgorithmProvider, IFileSystemUtils fileSystemUtils, IAssemblyConfig assemblyConfig)
        {
        ...
        public ContextAdapter SanitizeContext(ContextAdapter context)
        {
            if (context is null)
            {
            ...
            //context.HashAlgorithm = GetHashAlgorithmName(context);
            ...
            //context.ManifestDirPath = GetManifestDirPath(context.ManifestDirPath, context.BuildDropPath.Value, context.ManifestToolAction);

            // Set namespace value if provided in the assembly
            context.NamespaceUriBase = GetNamespaceBaseUriFromAssembly(context, logger);

            // Set default ManifestInfo for validation in case user doesn't provide a value.
            //context.ManifestInfo = GetDefaultManifestInfoForValidationAction(context);
        ...
        //private ConfigurationSetting<IList<ManifestInfo>> GetDefaultManifestInfoForValidationAction(IContext context)
        //{
        //    if (context.ManifestToolAction != ManifestToolActions.Validate
        //        || (context.ManifestInfo?.Value != null && context.ManifestInfo?.Value?.Count != 0))
        //    {
        //        return context.ManifestInfo;
        //    }

        //    var defaultManifestInfo = assemblyConfig.DefaultManifestInfoForValidationAction;
        //    if (defaultManifestInfo == null && (context.ManifestInfo.Value == null || context.ManifestInfo.Value.Count == 0))
        //    {
        ...
        private ConfigurationSetting<string> GetNamespaceBaseUriFromAssembly(IContext context, ILogger logger)
        {
        ...
        private ConfigurationSetting<string> GetPackageSupplierFromAssembly(IContext contxext, ILogger logger)
        {
            if (string.IsNullOrWhiteSpace(assemblyConfig.DefaultPackageSupplier))
            {
                return contxext.PackageSupplier;
            }

            // Give priority to package supplier provided as an argument.
            if (!string.IsNullOrWhiteSpace(contxext.PackageSupplier?.Value))
            {
                if (!string.IsNullOrWhiteSpace(assemblyConfig.DefaultPackageSupplier))
                {
