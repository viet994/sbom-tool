...
namespace Microsoft.Sbom.Api.Metadata
{
    ...
    public class LocalMetadataProvider : IMetadataProvider, IDefaultMetadataProvider
    {
        ...
        public LocalMetadataProvider(IConfiguration configuration, IContext context)
        {
        ...
        private void PopulateMetadata()
        {
            ...
            MetadataDictionary.AddIfKeyNotPresentAndValueNotNull(MetadataKey.PackageName, context.PackageName?.Value);
            MetadataDictionary.AddIfKeyNotPresentAndValueNotNull(MetadataKey.PackageVersion, context.PackageVersion?.Value);
            MetadataDictionary.AddIfKeyNotPresentAndValueNotNull(MetadataKey.PackageSupplier, context.PackageSupplier?.Value);

            // Add generation timestamp
            MetadataDictionary.AddIfKeyNotPresentAndValueNotNull(MetadataKey.GenerationTimestamp, context.GenerationTimestamp?.Value);
        }
        ...
        public string GetDocumentNamespaceUri()
        {
            ...
            var uniqueNsPart = Uri.EscapeDataString(context.NamespaceUriUniquePart?.Value ?? IdentifierUtils.GetShortGuid(Guid.NewGuid()));

            return string.Join("/", context.NamespaceUriBase.Value, packageName, packageVersion, uniqueNsPart);
        }
    }
